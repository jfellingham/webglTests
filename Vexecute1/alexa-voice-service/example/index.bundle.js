...
//  fd.append('fname', 'audio.wav');
  ...
  xhr.open('POST', 'https://localhost:9745/audio', true);
...
function getSpectrumData(data){
    freqData = data;
    
    var spec = MovingAverage(BandVol(frqLow, frqHigh)) * volume;
    //var spec = BandVol(frqLow, frqHigh);
    var count = 0; 
...
function BandVol(fLow, fHigh){
    fLow = clamp(fLow, 20, fMax);
    fHigh = clamp(fHigh, fLow, fMax);
    
     var n1 = Math.floor(fLow * nSamples / fMax);
     var n2 = Math.floor(fHigh * nSamples / fMax);
     var sum = 0;
       for (var i = n1; i <= n2; i++)
        {
            sum += freqData[i]/100;
...
function MovingAverage(sample){
        
    if (qSamples == 0)
        {
            filter = new Array(sizeFilter).fill(0.0);
        }
...
function preload(){
  ...
  window.sound = loadSound("test.wav");
}
function setup(){
  var cnv = createCanvas(100,100);
  cnv.mouseClicked(togglePlay);
  window.fft = new p5.FFT();

}
function getData(data){
if(data != undefined && data !=null){
   
unityInstance.SendMessage('FinalMayaModel', 'getSpectrumData', data/20-0.04);
}
else{
        unityInstance.SendMessage('demo', 'getSpectrumData', 0);
    }
}
...
function draw(){
  ...
  var spectrum = window.fft.analyze();
//  noStroke();
...
function togglePlay() {
      window.sound.amp(0.2);
  if (window.sound.isPlaying()) {
    window.sound.pause();
  } else {
...
avs.on(AVS.EventTypes.TOKEN_SET, function () {
  ...
  stopRecording.disabled = true;
     unityInstance.SendMessage('demo', 'changeLoginButton', 'Logout');
...
avs.on(AVS.EventTypes.LOGOUT, function () {
  ...
  startRecording.disabled = true;
  stopRecording.disabled = true;
     unityInstance.SendMessage('demo', 'changeLoginButton', 'Login');
              ...
              if (directive.name === 'speak') {
                ...
                if (audio) {
                  ...
                  avs.audioToBlob(audio).then(function (blob) {
                      ...
                      window.sound = loadSound(blob, function(){ 
    ...
    value: function playBlob(blob) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var audio = new Audio();
          ...
          window.sound = loadSound(objectUrl);
