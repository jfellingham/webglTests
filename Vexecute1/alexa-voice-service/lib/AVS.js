...
class AVS {
  ...
  sendAudio (dataView) {
    return new Promise((resolve, reject) => {
      ...
      const url1 = 'https://pitangui.amazon.com/api/cards?limit=1';

      xhr1.open('POST', url1, true);
      xhr1.responseType = 'arraybuffer';
      xhr1.onload = (event) => {
          
          const buffer1 = new Buffer(xhr1.response);
          if (xhr1.status === 200) {
          const parsedMessage = httpMessageParser(buffer1);
          resolve({xhr1, response: parsedMessage});
          }
          else{
